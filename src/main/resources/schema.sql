drop table if exists users, friends , mpa, films, genres, films_genres , film_likes, directors, films_directors,
 reviews, review_useful, events;
...
create table if not exists reviews (
    id integer by default as identity not null primary key,
    content varchar (200),
    is_positive boolean,
    user_id integer references users (id) on delete cascade,
    film_id integer references films (id) on delete cascade,
    userful integer not null  -- возможно эта колонка не нужна и будем рассчитывать рейтинг через JOIN
)
...
create table if not exists events (
    event_id integer by default as identity not null primary key,
    time_stamp timestamp,
    user_id integer references users (id) on delete cascade,
    event_type enum -- пока так, надо еще подумать
    operation enum -- пока так, надо еще подумать
